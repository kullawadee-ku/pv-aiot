# =====================================================================
# engine.py  —  PV-IoT Simulation Engine
# KMUTT PV-IoT Workshop
# Version : 1.0.0
# License : MIT
# =====================================================================
...
if __name__ == "__main__":
    ...
    # ── Mode selector ────────────────────────────────────────────────
    # Options:
    #   'single'   → simulate one season, plot & export CSV
    #   'compare'  → simulate all three seasons, overlay comparison plot
    #   'all'      → simulate all three, export individual CSVs + comparison plot

    RUN_MODE = 'compare'          # ← change to 'single' or 'all'
    SINGLE_SEASON = 'rainy'       # ← only used when RUN_MODE = 'single'

    TIME_RESOLUTION_MINS = 5.0

    # ── Helper: build engine + run one season ───────────────────────
    def run_season(season: str) -> list:
    ...
    # ── RUN ──────────────────────────────────────────────────────────
    if RUN_MODE == 'single':
        data = run_season(SINGLE_SEASON)
        save_simulation_to_csv(data, f"G-SET_{SINGLE_SEASON.upper()}.csv")
        plot_single_season(data, SINGLE_SEASON, f"G-SET_{SINGLE_SEASON.upper()}_Report.png")
        print_financial_summary(SINGLE_SEASON, data)

    elif RUN_MODE in ('compare', 'all'):
        all_data = {}
        for s in SeasonPreset.list_seasons():
            all_data[s] = run_season(s)
            if RUN_MODE == 'all':
                save_simulation_to_csv(all_data[s], f"G-SET_{s.upper()}.csv")
                plot_single_season(all_data[s], s, f"G-SET_{s.upper()}_Report.png")
            print_financial_summary(s, all_data[s])

        plot_season_comparison(all_data, save_img="G-SET_Season_Comparison.png")

    else:
        print(f"Unknown RUN_MODE '{RUN_MODE}'. Use 'single', 'compare', or 'all'.")
